...
class pearlData {
    ...
    {		
         shortcode_atts( array(
            ...
            $lightbox = ( $atts['lightbox'] == 'yes') ? 1 : 0;
            ...
            {
                    $pearl_rss_feed_display = $this->parseRSS( $doc, $rss_url, $lightbox );
            }	
    ...
    public function parseRSS( $xml, $rss_url, $lightbox )
    {
       
        $html = "<div class='pearl_rss'> ";
        $id = 'pearl-lightbox-'.rand(5, 15);
        if( $lightbox ) {
            $html .= '<a href="#'.$id.'" class="popup-modal" >';
            $html .= $xml->channel->title;
            $html .= '</a>';
            $html .= '<div id="'.$id.'" class="mfp-hide white-popup-block pearl-lightbox-popup">';
        ...
        {   
            ...
            {	
               ...
               {
                   //style="background-image: url('.$thumb['url'].');width: '.$thumb['width'].'px;height: '.$thumb['height'].'px;"
                    $media = $rssData->children('media',true);
                    $thumb = $media->thumbnail[0]->attributes();
                    $html .= '<div class="pearl_rss_left" >'
                            . '<img src="'.$thumb['url'].'" alt=""></div>';            
               }
        ...
        if( $lightbox ) {
            $html .= ' <p><a class="popup-modal-dismiss" href="#">Close Window</a></p>
            </div>';
        }
        $html .= "</div>" ;
        return $html;
